...
namespace ExpansionImage
{
	...
	public class CustomUV : BaseMeshEffect
	{
		...
		public override void ModifyMesh(VertexHelper vh)
		{
			...
			if (image is ExpansionImage)
			{
				ExpentionSimpleSprite(vh);
				return;
			}
			switch (image.type)
			{
		...
		private void ExpentionSimpleSprite(VertexHelper vh)
		{
			...
			for (int i = 0; i < currentVertCount; i++)
			{
				...
				vertex.uv0.x = vertex.uv0.x * (_uvRect.width + 1f) + _uvRect.x;
				vertex.uv0.y = vertex.uv0.y * (_uvRect.height + 1f) + _uvRect.y;
