...
{
	public class ModeSelectButtonController : ButtonControllerBase
	{
		[SerializeField]
		...
		public override void Initialize(int monitorIndex)
		{
			...
			type = FlatButtonType.Setting;
			CommonButtons[8] = Object.Instantiate(CommonPrefab.GetFlatButtonObject(), _positions[8]);
			CommonButtons[8].Initialize(monitorIndex, InputManager.ButtonSetting.Button08, ButtonControllerBase.GetFlatButtonParam(type).LedColor);
			CommonButtons[8].SetSymbol(ButtonControllerBase.GetFlatButtonParam(type).Image, isFlip: false);
			CommonButtons[8].SetSE(ButtonControllerBase.GetFlatButtonParam(type).Cue);
		}
		...
		public void ChangeButtonType(FlatButtonType type)
		{
			...
			case FlatButtonType.Cancel:
				...
				CommonButtons[2].SetSE(ButtonControllerBase.GetFlatButtonParam(type).Cue);
				break;
			case FlatButtonType.Setting:
				CommonButtons[8].SetSymbol(ButtonControllerBase.GetFlatButtonParam(type).Image, isFlip: false);
				CommonButtons[8].ChangeColor(ButtonControllerBase.GetFlatButtonParam(type).LedColor);
				CommonButtons[8].SetSE(ButtonControllerBase.GetFlatButtonParam(type).Cue);
				break;
			case FlatButtonType.SettingBack:
				CommonButtons[8].SetSymbol(ButtonControllerBase.GetFlatButtonParam(type).Image, isFlip: false);
				CommonButtons[8].ChangeColor(ButtonControllerBase.GetFlatButtonParam(type).LedColor);
				CommonButtons[8].SetSE(ButtonControllerBase.GetFlatButtonParam(type).Cue);
				break;
		...
		public void ChangeBottonSE(FlatButtonType type, bool isChange = false)
		{
			if (type == FlatButtonType.Ok)
			{
				if (isChange)
				{
					CommonButtons[0].SetSE(Cue.SE_ENTRY_FIX);
				}
				else
				{
					CommonButtons[0].SetSE(ButtonControllerBase.GetFlatButtonParam(type).Cue);
