...
public class Triangulator
{
	...
	public int[] Triangulate()
	{
		...
		int index2 = list[1];
		int index3 = list[2];
		Vector3 vector = new Vector3(m_points[index].x, m_points[index].y, 0f);
		Vector3 vector2 = new Vector3(m_points[index2].x, m_points[index2].y, 0f);
		Vector3 vector3 = new Vector3(m_points[index3].x, m_points[index3].y, 0f);
		Vector3 lhs = vector2 - vector;
		Vector3 rhs = vector3 - vector2;
		Vector3 rhs2 = Vector3.Cross(lhs, rhs);
		if (Vector3.Dot(m_cameraPosition, rhs2) > 0f)
		{
			flag = false;
		}
		if (flag)
